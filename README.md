# CSS 学习笔记

## 基础
- [position | 元素定位](./note/position.md)
- [float | 元素浮动](./note/float.md)
- [BFC | 块级格式化上下文](./note/BFC.md)
- [transition | 过渡属性](./note/transition.md)
- [transform | 形变属性](./note/transform.md)
- [border | 边框属性 ](./note/border.md)
- [box-shadow | 阴影属性 ](./note/box-shadow.md) 
- [filter | 滤镜属性](./note/filter.md)
- [flex | 一维布局方式](./note/flex.md)
- [grid | 二维布局方式](./note/grid.md)
- [mask | 遮罩属性](./note/mask.md)
- [clip-path | 图形裁剪](./note/clip-path.md)
- [animation | 动画效果](./note/animation.md)
- [选择器](./note/selectors.md)
- [伪类](./note/伪类.md)
- [伪元素](./note/伪元素.md)
- [盒模型](./note/box-model.md)
- [渐变属性](./note/渐变属性.md)
- [属性的继承](./note/属性的继承.md)
- [元素单位](./note/元素单位.md)
- [图片适应 | object-fit](./note/图片适应.md)


## 代码
- [网格布局实例 | grid 属性](./src/style/grid-layout.css)
- [实现两栏布局方法 | 1，flex 属性；2，position 属性](./src/style/two-columns.css)
- [实现三栏布局方法 | 1，flex 属性；2，position 属性](./src/style/three-columns.css)
- [阴影效果 | box-shadow 属性](./src/style/box-shadow.css)
- [渐变背景 | 1，line-gradient；2，radial-gradient](./src/style/gradient.css)
- [卡片翻转 | backface-visibility](/src/style/turn-card.css)
- [磨砂玻璃背景效果 | filter 属性](./src/style/frosted-glass-bg.css)
- [卡片展开 | transform 属性](./src/style/card-clip-extend.css)


